...
(function deepResearch() {
  ...
  function claimEl(c) {
    ...
    const meas = c.value
      ? `<span class="cmeas">${escapeHtml(c.metric || "measured")} = ${escapeHtml(c.value)}</span>`
      : "";
    const cond = c.condition ? `<span class="ccond">${escapeHtml(c.condition)}</span>` : "";
    div.innerHTML =
      ...
      `<span class="cclaim">${escapeHtml(c.claim || "")}</span>${meas}${cond}`;
    return div;
  }
